...
def authorized_client(client: AsyncClient, test_user: UserInDB) -> AsyncClient:
    access_token = jwt.create_access_token_for_user(user=test_user, secret_key=str(SECRET_KEY))
...
@pytest.fixture
async def test_admin(db: Database) -> UserInDB:
    new_user = UserCreate(
        email="johnny@depp.dev",
...
@pytest.fixture
async def test_librarian(db: Database) -> UserInDB:
    new_user = UserCreate(
        email="lebron@james.basketball",
        username="lebron_james",
        password="lalakers",
        role=UserRole.librarian,
        library_card_number="7654321",
    )
    return await user_fixture_helper(db=db, new_user=new_user)
...
async def test_user3(db: Database) -> UserInDB:
    new_user = UserCreate(email="bruce@lee.dev", username="bruce-lee", password="kowloon")
    return await user_fixture_helper(db=db, new_user=new_user)


@pytest.fixture
async def test_user4(db: Database) -> UserInDB:
    new_user = UserCreate(email="jennifer@lawrence.io", username="jlaw", password="mockingbird")
    return await user_fixture_helper(db=db, new_user=new_user)
...
def create_authorized_client(client: AsyncClient) -> Callable:
    def _create_authorized_client(*, user: UserInDB) -> AsyncClient:
        access_token = jwt.create_access_token_for_user(user=user, secret_key=str(SECRET_KEY))
        client.headers = {
            **client.headers,
            "Authorization": f"{JWT_TOKEN_PREFIX} {access_token}",
        }
