...
class TestUserLogin:
    async def test_user_can_login_successfully_and_receives_valid_token(
        ...
        login_data = {
            ...
            "password": "saginaw",  # insert user's plaintext password
        }
        res = await client.post(app.url_path_for("auth:login-existing-user"), data=login_data)
    ...
    async def test_user_with_wrong_creds_doesnt_receive_token(
        ...
        user_data["password"] = "saginaw"  # insert user's plaintext password
...
class TestAuthTokens:
    async def test_can_create_access_token_successfully(
        ...
        access_token = jwt_service.create_access_token_for_user(
            user=test_user,
    ...
    async def test_can_retrieve_username_from_token(
        ...
        token = jwt_service.create_access_token_for_user(user=test_user, secret_key=str(SECRET_KEY))
        username = jwt_service.get_username_from_token(token=token, secret_key=str(SECRET_KEY))
    ...
    async def test_error_when_token_or_secret_is_wrong(
        ...
        with pytest.raises(HTTPException):
            username = jwt_service.get_username_from_token(token=wrong_token, secret_key=str(secret))
...
class TestUserRegistration:
    ...
    async def test_users_saved_password_is_hashed_and_has_salt(
        ...
        user_repo = UsersRepository(db)
        new_user = {"email": "beyonce@carter.io", "username": "knowles", "password": "jayz123"}
        ...
        assert user_in_db.verify_password(new_user["password"])
